...
export function RunPayrollTab({ activeRoster, profiles, allPatients, allPayments, onSaveProfile, onAddPayment }) {
  ...
  const [paidMyopia, setPaidMyopia] = useState({});

  // Load paid myopia bonus records from Firebase
  React.useEffect(() => {
    const loadPaid = async () => {
      try {
        ...
        for (const d of activeRoster) {
          const snap = await getDocs(collection(db, 'doctorProfiles', d.id, 'myopiaBonusPaid'));
          snap.forEach(doc => { if (doc.data().paid) paid[doc.id] = true; });
        }
        setPaidMyopia(paid);
      } catch(e) { console.error(e); }
    };
    if (activeRoster.length > 0) loadPaid();
  }, [activeRoster.length]);
  ...
  const runPayroll = async () => {
    ...
    try {
      ...
      console.log('[Payroll] OCR response:', JSON.stringify(schedData).slice(0, 500));
      ...
      const periodYear = periodStart.slice(0, 4); // e.g. "2026"
      console.log('[Payroll] Period:', periodStart, '→', periodEnd);
      console.log('[Payroll] initialsMap:', initialsMap);
      const assignments = [];
      for (const week of schedData.weeks) {
        for (let di = 0; di < 7; di++) {
          // Fix year if OCR returned wrong year (e.g. 2025 instead of 2026)
          let dateStr = week.dates[di];
          if (!dateStr) continue;
          if (dateStr.slice(0, 4) !== periodYear) {
            dateStr = periodYear + dateStr.slice(4);
          }
          const d = new Date(dateStr + 'T12:00:00');
          if (d < pStart || d > pEnd) continue;
          for (const loc of ['SC','SV','F','WC']) {
            const rawCell = (week[loc]?.[di] || '').trim();
            if (!rawCell || rawCell === '' || rawCell === 'BLOCKED') continue;
            const cell = rawCell
              .replace(/\s*\([\d:.\-]+.*?\)/g, '')
              .replace(/\s+[\d]+[:\-][\d:.\-]+.*$/, '')
              .trim();
            console.log('[Payroll] cell:', rawCell, '→', cell, 'match:', initialsMap[cell]||'NONE');
            if (!cell) continue;
      ...
      console.log('[Payroll] assignments:', assignments.length, assignments.slice(0,5));
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {results && (
        <div style={{ background: 'white', borderRadius: 14, padding: '16px 20px', boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
          ...
          {/* Myopia Control Bonuses section */}
          {(()=>{
            const myopiaEntries = (allPatients||[]).filter(p => {
              if (!(p.hasMyopia || parseFloat(p.myopiaAmt||p.myopia) > 0)) return false;
              const d = p.date||'';
              return d >= periodStart && d <= periodEnd;
            });
            if (myopiaEntries.length === 0) return null;
            // Group by doctor
            const byDoc = {};
            for (const p of myopiaEntries) {
              const docId = p.doctor||p.doctorId||'';
              if (!docId||docId==='Yang') continue;
              if (!byDoc[docId]) byDoc[docId] = [];
              byDoc[docId].push(p);
            }
            if (Object.keys(byDoc).length === 0) return null;
            return (
              <div style={{ marginBottom: 20, background: '#F5F3FF', border: '1px solid #DDD6FE', borderRadius: 10, padding: '12px 16px' }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#6D28D9', marginBottom: 10 }}>⚡ Myopia Control Bonuses to Pay</div>
                {Object.entries(byDoc).map(([docId, pts]) => {
                  const doctor = activeRoster.find(d => d.id === docId);
                  return (
                    <div key={docId} style={{ marginBottom: 10 }}>
                      <div style={{ fontSize: 12, fontWeight: 700, color: '#1B3A5C', marginBottom: 4 }}>{doctor?.name || docId}</div>
                      {pts.map((p, i) => {
                        const amt = parseFloat(p.myopiaAmt||p.myopia||0);
                        const paidKey = `myopia_${p.date}_${(p.patientName||'').replace(/\s/g,'_')}`;
                        const isPaid = paidMyopia[paidKey];
                        return (
                          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 8px', background: isPaid?'#F0FDF4':'white', borderRadius: 5, marginBottom: 2, border: `1px solid ${isPaid?'#6EE7B7':'#E2E8F0'}` }}>
                            <input type="checkbox" checked={!!isPaid} onChange={async e => {
                              const upd = { ...paidMyopia, [paidKey]: e.target.checked };
                              setPaidMyopia(upd);
                              // Save to Firebase
                              const { doc, setDoc } = await import('firebase/firestore');
                              const { db } = await import('./firebase');
                              await setDoc(doc(db, 'doctorProfiles', docId, 'myopiaBonusPaid', paidKey), { paid: e.target.checked, date: p.date, patient: p.patientName, amt, updatedAt: new Date().toISOString() });
                            }} style={{ cursor: 'pointer' }} />
                            <span style={{ fontSize: 11, color: '#1B3A5C', flex: 1, textDecoration: isPaid?'line-through':'none' }}>
                              {p.patientName} — {p.date?.slice(5).replace('-','/')} — {p.loc}
                            </span>
                            <span style={{ fontSize: 11, fontWeight: 700, color: '#6D28D9' }}>{amt > 0 ? `$${amt}` : 'Myopia Ctrl'}</span>
                          </div>
                        );
                      })}
                    </div>
